...
outputdir = '/project/p/pjk/lfl/ramip/snp_data' # Pickle files saved here
...
try:
    if recalculate_eofs:
        raise Exception(
            'recalculate_eofs is True, so EOFs will be re-loaded and pickled')
    eofs, variances = pickle.load(
        open(f"{outputdir}/{name}_{varnam}_EIG.p","rb"))
except:
    cov_mat = np.matmul(X_flat.values.T, X_flat.values)/(n-1)
...
N_EOFS = 200 # number of EOFs retained in S/N maximizing pattern analysis

# Large Ensemble Forced Patterns
eofs_scaled = np.matmul(eofs[:, 0:N_EOFS], np.diag(1/sigmas[0:N_EOFS]))
eofs_scaled_adj = np.matmul(
    np.diag(sigmas[0:N_EOFS]), eofs[:, 0:N_EOFS].T)

# ensemble-mean principal components
pcs_ensmean = np.matmul(X_ensmean_flat.values, eofs_scaled)

# covariance matrix of ensemble-mean principal components
s_mat = np.cov(pcs_ensmean.T)

u2, signal_frac, v2 = np.linalg.svd(s_mat)

SNP = np.matmul(v2, eofs_scaled_adj)
SNPs_reshaped = (
    SNP.reshape(N_EOFS, len(lat), len(lon))/scale.values[None, :, None])

weights = np.matmul(eofs_scaled, v2.T)
weights = (
    weights.reshape(len(lat), len(lon), N_EOFS)*scale.values[:, None, None])

weights = weights.reshape(len(lat)*len(lon), N_EOFS)

# compute timeseries from full data matrix
tk = np.matmul(Xt_flat.values, weights)
# compute ensemble-mean timeseries from ensemble-mean data
tk_emean = np.matmul(Xt_ensmean_flat.values, weights) 
...
tk_reshape = tk.reshape(nt, len(ne), N_EOFS)
